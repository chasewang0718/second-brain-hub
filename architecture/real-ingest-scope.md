...
| 2026-04-22 | **B-ING-5 扩容（helper chats）**：新增 `wechat-sync --include-helper-chats`（默认关闭，显式开启后纳入 `chat_file_transfer_assistant.json` / `chat_filehelper.json`）。先 dry-run 评估 `would_insert=100`，再以快照 `20260422-112619-bing5-helper-chats.duckdb`（sha `45d87a0f...`）保护后 apply；实际写入 `inserted=50`（另 50 因同 `source_id` 去重为 `skipped_existing`）。复跑 dry-run 显示三会话均 `would_insert=0`，幂等成立。 |
| 2026-04-22 | **B-ING-5 噪音控制阀（会话白/黑名单 + helper 绑定模式）**：`wechat-sync` 新增 `--chat-whitelist` / `--chat-blacklist`（按 `conversation` 过滤）与 `--helper-chat-mode link-person|no-person`（helper 会话是否挂 person）。dry-run 验证：黑名单 `filehelper` 可跳过两份 helper 会话；白名单 `filehelper` + `no-person` 可只纳入 helper 且不关联联系人，满足“保留记录但不污染人物上下文”。 |
