...
| 2026-04-22 | **B-ING-1.11 / 1.10 / 1.4 / 1.5 / 1.9 / 1.6 / 1.7 ✅**：7 条 follow-up 连续落地。`merge_persons` 自动合并 aliases_json、`identifiers-repair --dry-run` 纯净化 + pair 去重、`ios_backup_locator` 按 basename+size 排序避开空子库、pytest 切 `BRAIN_DB_PATH` 隔离 + 清 198 条 T-fixture 污染、`contacts-ingest-ios` 幂等冒烟、`merge-candidates enqueue-manual` 子命令、`ingest_events.backup` 自动回填最近 snapshot。全量 pytest **232/232**。详情见 `bing1-followups.md`。 |
