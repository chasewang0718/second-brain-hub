...
| 2026-04-21 | A3 音频分支落地：`brain_agents/audio_inbox.py` + `brain audio-inbox-ingest`（faster-whisper 懒加载，`BRAIN_ASR_MODEL` / `BRAIN_ASR_LANG` 可调；缺库写 `asr_status: pending` 指针卡 + cursor_queue 兜底）；新增 `config/paths.yaml:audio_inbox_dir` 与 6 个 pytest 用例（全量 42）。Caps+D 音频分支同样走 `CF_HDROP` 扩展名分派。 |
| 2026-04-21 | Caps+D 三件套统一分派：新增 `tools/ps/brain-caps-d-dispatch.ps1`（`Invoke-BrainCapsDSave`），按扩展名把剪贴板文件列表批量路由到 `brain pdf/image/audio-inbox-ingest`；`.reference` profile 里的 `gsave` 前置一行 `if (Invoke-BrainCapsDSave) { return }`，未识别 / 无文件降级文本分支，零行为变化。新增 `test_caps_d_dispatch_sync.py` 4 用例守护 PS 映射表与 `brain_agents.{image,audio}_inbox.SUPPORTED_EXT` 同步（全量 46）。 |
| 2026-04-21 | A4 provenance 落地：`write_assist.write_draft` 尾部自动追加 `## 参考` 块（编号 · kind · title · sha256 · person），并用 `enrich_provenance` 从目标 md 的 frontmatter 提取 `asset_sha256 / asset_type / person_id / ocr/asr_status`；`kind` 按路径段（`inbox-auto-pdf/image/audio`, `05-contacts/`, `04-journal/`）分类；`include_provenance=False` 可旁路；6 个 pytest 用例（全量 52）。 |
