...
mirror_at: D:\brain\01-concepts\workflow\brain-tools-index.md
---

# second-brain-hub · 控制中枢的架构索引

> **📍 权威副本**: `C:\dev-projects\second-brain-hub\brain-tools-index.md`
> 镜像: `D:\brain\01-concepts\workflow\brain-tools-index.md`
>
...
│   ├── brain-asset-migrate.ps1     外部资产扫描 / 迁移 (Tier B 批量处理)
...
│   ├── brain-asset-overview-cards.ps1   Tier B 叶子目录 → Tier A 总览卡生成 (agent)
...
**CapsLock+D** → 剪贴板 → `D:\brain\99-inbox\paste-YYYYMMDD-HHMMSS.md`
...
| `gasset` | 查看 `D:\brain-assets\` 现状（Tier B 容量 / 分布） | assets |
...
**日志**：`D:\brain\.brain-weekly.log`（已进 `.gitignore`）
**锁文件**：`D:\brain\.brain-weekly.lock`（防重复运行）
...
- 输出：`D:\brain-assets\_migration\<job>-manifest.tsv`
...
| 脚本 | 输入 | 输出 | 耗时 |
|---|---|---|---|
| `ghealth` | `D:\brain` | `04-journal/brain-health-YYYY-MM-DD.md` | 秒级 |
| `gdedup` | `D:\brain-assets` | `_migration/dedup-YYYY-MM-DD.md` + `.tsv` | 依文件数，百 MB 几分钟 |
| `gstats` | `D:\brain-assets` | `04-journal/brain-assets-stats-YYYY-MM-DD.md` | 秒级 |
...
### `powershell/brain-asset-overview-cards.ps1`

Tier B 叶子目录 → Tier A 总览卡。扫 `brain-assets/` 找有 ≥ 3 文件的叶子目录，如果 Tier A 对应位置没 `README.md`/`overview.md`，让 agent **只看文件名**（不读内容）生成一张集群总览卡。

| 参数 | 默认 | 说明 |
|---|---|---|
| `-MinFiles` | 3 | 叶子目录至少多少个文件才补卡 |
| `-MaxItems` | 5 | 单次最多处理几个目录 |
| `-Execute` | 关 | 默认 dry-run，加 `-Execute` 才真调 agent |

**Token 预算**：每张卡 ~1-3k token（只读文件名，不读内容），20 张卡 ≈ 30-60k token。

### `powershell/brain-nightly-push.ps1` + `register-nightly-push-task.ps1`

每晚 **22:00** Task Scheduler 自动跑，把 `D:\brain` 和 `C:\dev-projects\brain-tools` 的本地 commit 推到 GitHub。

- **只 push 不 commit**（commit 由人/agent 主动触发，保证粒度）
- 无未推送 commit 就静默跳过，不打扰
- 日志：`D:\brain\.brain-nightly-push.log`（已 gitignore）
...
# 3. clone 两个仓库
git clone https://github.com/chasewang0718/chase-brain.git D:\brain
git clone https://github.com/chasewang0718/chase-brain-tools.git C:\dev-projects\brain-tools
